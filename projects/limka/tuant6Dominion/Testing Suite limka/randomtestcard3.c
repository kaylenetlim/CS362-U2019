...
int main(){
   ...
   for(int i = 0; i < MAX_TEST; i++){
      ...
      int testret = cardEffect(tribute, choice1, choice2, choice3, &teststate, handPos, &bonus); // Holds return value of cardEffect
	 ...
	 fail++;
      ...
      if(state.numActions == (teststate.numActions + 2)){
	 printf("PASSED... action points increase by 2\n");
	 pass++;
      }else{
	 ...
	 fail++;
      ...
      // Test if buys increase
      printf("\n>>> Testing if number of buys increase... ");

      if(state.numBuys == (teststate.numBuys + 2)){
	 printf("PASSED... buys increase by 2\n");
	 pass++;
      }else{
